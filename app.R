...
read_upload <- function(path) {
  ...
  secondary <- c("No.", "no.", "type", "Type", "Class", "class",
  ...
  df <- tryCatch({
    ...
    d  <- read.table(path, header = TRUE, fill = TRUE,
...
run_fb_mcmc <- function(X_ref, y_ref,
  ...
  log_post <- function(beta, rho) {
    ...
    if (any(!is.finite(mu)) || gamma <= 0) return(-Inf)
  ...
  for (iter in seq_len(n_iter)) {
    ...
    if (iter <= burn_in && iter %% tune_every == 0L) {
      for (k in seq_len(n_beta)) {
        ...
        if      (rate > 0.44) sigma_beta[k] <- sigma_beta[k] * 1.3
        else if (rate < 0.20) sigma_beta[k] <- sigma_beta[k] * 0.7
        acc_beta[k] <- 0L
      }
      rate_rho <- acc_rho / tune_every
      if      (rate_rho > 0.44) sigma_rho <- sigma_rho * 1.3
...
ui <- fluidPage(
  ...
  tags$head(tags$style(HTML("
    ...
    h4 { color: #333; border-bottom: 1px solid #ddd; padding-bottom: 4px; margin-top: 18px; }
    .btn-run { width: 100%; margin-top: 12px; }
  "))),

  titlePanel("RAPTOR Scheme Evaluation"),
    ...
    mainPanel(
      tabsetPanel(id = "main_tabs",
        ...
        tabPanel("Results Plot",
          br(),
          plotOutput("results_plot", height = "550px")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  ref_data <- reactive({
    req(input$ref_file)
    read_upload(input$ref_file$datapath)
  })

  trt_data <- reactive({
    req(input$trt_file)
    read_upload(input$trt_file$datapath)
  })
  ...
  output$trt_after_ui <- renderUI({
    ...
    selectInput("trt_after_col", "After count column(s)",
                choices  = cols,
                selected = cols[min(5, length(cols))],
                multiple = TRUE)
  })
  ...
  output$cov_match_ui <- renderUI({
    ...
    trt_cols <- setdiff(colnames(trt_data()),
                        c(input$trt_id_col,
  ...
  results <- eventReactive(input$run_btn, {
    ...
    col_sum <- function(df, cols) {
      if (length(cols) == 1) return(to_num(df[[cols]]))
  ...
  output$results_summary <- renderUI({
    ...
    div(
      ...
      div(style = "background:#eef2ff; padding:12px 20px; border-radius:4px; flex:1; text-align:center;",
          div(style = "font-size:1.4em; font-weight:bold;", total_raw),
          div("Total raw change")),
      div(style = "background:#fff7e6; padding:12px 20px; border-radius:4px; flex:1; text-align:center;",
          div(style = "font-size:1.4em; font-weight:bold;", round(total_rtm, 1)),
          div("Change due to RTM")),
      div(style = "background:#edfff0; padding:12px 20px; border-radius:4px; flex:1; text-align:center;",
  ...
  output$results_table <- renderDT({
    req(results())
    df          <- results()
    names(df)   <- c("Site", "Before", "After", "RTM Estimate (post. mean)",
  ...
  output$results_plot <- renderPlot({
    ...
    plot_df$Site <- factor(plot_df$Site,
                           levels = as.character(df$Site))

    ggplot(plot_df, aes(x = Site, y = Value, fill = Component)) +
      geom_col(position = "stack", width = 0.7) +
      geom_hline(yintercept = 0, colour = "black", linewidth = 0.4) +
      scale_fill_manual(
        values = c("RTM Change"       = "#5B9BD5",
                   "Treatment Change" = "#ED7D31")
      ) +
      labs(
        title = "Change in Collisions by Site: RTM vs Treatment Effect",
        x     = "Site",
        y     = "Change in collisions",
        fill  = NULL
      ) +
